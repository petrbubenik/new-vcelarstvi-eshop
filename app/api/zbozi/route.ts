...
async function getZboziFeed(): Promise<string> {
  ...
  for (const product of products) {
    ...
    for (const variant of product.variants) {
      ...
      // Build PRODUCT field (manufacturer + product name + variant details)
      const productField = `${manufacturer} ${product.name}${variantParts.length > 0 ? ` – ${variantParts.join(" ")}` : ""}`;
  ...
  const xmlItems = items.map((item) => {
    ...
    return `      <SHOPITEM>
        ...
        <PRODUCT>${escapeXml(item.PRODUCT)}</PRODUCT>
...
export async function GET() {
  try {
    ...
    console.error("Zbozi feed error:", error);
